...
export default function AuthButtons({session}) {
    ...
    if (!session) {
        return (
            <div className="flex items-center gap-2">
                <Link href="/login" className="px-4 py-1.5 text-sm font-medium text-white bg-gray-800 rounded-full hover:bg-gray-700 transition">Iniciar sesión</Link>
                <Link href="/register" className="px-4 py-1.5 text-sm font-medium text-gray-800 border border-gray-300 rounded-full bg-white hover:bg-gray-100 transition">Registrarse</Link>
            </div>
        );
    }
    ...
    return (
        <div className="relative" ref={menuRef}>
            <button onClick={() => setOpen(!open)} className=" cursor-pointer flex items-center gap-2 px-2 py-1 rounded-full hover:bg-gray-200 transition">
                {session.user?.image && (
                    <Image
