...
export default function FormularioTipoImagen({ gameId, user }) {
  const [state, action, pending] = useActionState(createImageContentAction, {});
  const router = useRouter();

  useEffect(() => {
    if (state?.success) {
      router.push("/juego?gameid=" + gameId); // redirige al listado o a donde quieras
    }
  }, [state]);
      ...
      <div className="bg-white dark:bg-slate-800 p-10 rounded-3xl shadow-2xl w-full space-y-6">
        ...
        <form className="space-y-6" action={action}>
          <div className="flex flex-col md:flex-row md:space-x-6 space-y-6 md:space-y-0">
            ...
            <input type="text" hidden name="userId" defaultValue={user?.id} />
            <input type="text" hidden name="gameId" defaultValue={gameId} />
            <input type="text" hidden name="type" defaultValue="IMAGEN" />
          ...
          <div className="pt-4">
            <button
              ...
              disabled={pending}
            >
              {pending ? "Subiendo imagen ..." : "Publicar"}
            </button>
          </div>
        </form>
      </div>
